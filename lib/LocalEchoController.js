        ...
        case "\t": // TAB
          if (this._autocompleteHandlers.length > 0) {
            ...
            if (candidates.length === 0) {
              ...
              // Just a single candidate: replace input with the candidate,
              // wrapping it in quotes if it contains a space.
              const lastToken = getLastToken(inputFragment);
              const hasSpace = candidates[0].includes(" ");
              const replaced =
                this._input.slice(0, this._input.length - lastToken.length) +
                (hasSpace ? '"' : "") +
                candidates[0] +
                (hasSpace ? '"' : "");
              this.setInput(replaced);
              this.setCursor(replaced.length);
            } else if (candidates.length <= this.maxAutocompleteEntries) {

              // search for a shared fragement
              const lastToken = getLastToken(inputFragment);
              const sameFragment = getSharedFragment(
                lastToken || inputFragment,
                candidates
              );
              ...
              if (sameFragment) {
                this.handleCursorInsert(
                  lastToken
                    ...
                    : sameFragment.substr(inputFragment.length)
                );
              }
